...
function Tours(): JSX.Element {
  ...
  location.length === 0 || location.includes(tour.location ?? "");
    ...
    const matchesAttendees =
      (minAttendees === "" ||
        (tour.attendees !== undefined &&
          tour.attendees >= Number(minAttendees))) &&
      (maxAttendees === "" ||
        (tour.attendees !== undefined &&
          tour.attendees <= Number(maxAttendees)));
    const matchesDate =
      (startDate === "" || new Date(tour.date) >= new Date(startDate)) &&
      (endDate === "" || new Date(tour.date) <= new Date(endDate));

    const matchesSearch = tour.tourname
      .toLowerCase()
    ...
    <div className="container my-5">
      ...
      {loading ? (
        ...
        <p className="text-center">No tours available? <br />
            <Link to="/tour/create"> create</Link> ur own adventure</p>
      )}
    </div>
  );
}
