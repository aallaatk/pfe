...
interface DashboardToursProps {
  guiderId: string; // Pass userId as a prop
}

const GuiderDashboardCreatedTours: React.FC<DashboardToursProps> = ({ guiderId }) => {
  ...
  const fetchTours = async () => {
    try {
      const response = await axios.get<Tour[]>(`http://localhost:3000/api/tours/guider/${guiderId}`);
...
export default GuiderDashboardCreatedTours;
