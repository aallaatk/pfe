...
const DashboardReservedTours: React.FC<DashboardReservedToursProps> = ({ userId }) => {
  ...
  const stripePromise = loadStripe("pk_test_51PKPiKRwQL00CT5o7dHPgJduEFOeUOvhNdfnfJkQnse5FkjS0GtrHHpkTmVUZhCObndzFwAdWnKAU22nEoupga0400XibCrGkr");
  useEffect(() => {
    fetchReservedTours();
  }, []);
  ...
  const fetchReservedTours = async () => {
    try {
      const response = await axios.get<Tour[]>(`/api/tours/reserved/guider/${userId}`);
  ...
  return (
    <div className="tour-list">
      ...
      <div className="text-center mt-4">
        <p style={{fontSize:'18px'}}>Total Price: <span style={{ fontWeight: 'bold', fontSize: '1.2rem' }}>{totalPrice} dt</span></p>
